...
function hasStrongPlanEvidence(record) {
  ...
  if (title.length <= 90 && /間取りの(?:１|1|２|2|３|3|一|二|三)?階部分|注文住宅の間取り|間取り図plan|平面図|図面|平屋.*間取り(?:事例|プラン)|間取り(?:事例|プラン|集)|間取り\s*(?:例|一覧|アーカイブ)|plan gallery|floor[-_ ]?plan archive/i.test(title)) return true;
...
function hasStrongImagePlanEvidence(record) {
  ...
  return /madori|floor[-_ ]?plan|floorplan|topview|heimen|hemen|zumen|drawing|間取り図|平面図|図面|plan[_-]?[0-9]|pic_small_pl_p[0-9]|collection_plan|madori_thm|zu[0-9]/i.test(`${fileName} ${alt}`);
}
...
function hasStrongFilePlanEvidence(record) {
  ...
  return /madori|floor[-_ ]?plan|floorplan|topview|heimen|hemen|zumen|drawing|間取り図|平面図|図面|plan[_-]?[0-9]|pic_small_pl_p[0-9]|collection_plan|madori_thm|zu[0-9]/i.test(fileName);
}
